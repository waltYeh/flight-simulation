...
#include <r2sim/commander_plot.h>
...
void commandsCallback(const r2sim::commands msg)
{
...
void statesCallback(const r2sim::states msg)
{
...
int main(int argc, char **argv)
{
	...
	float t_plan = 0.0;
//	jopt jopt1;
	ros::init(argc, argv, "simcommander");
	ros::NodeHandle n;
	ros::Publisher commander_plot_pub = n.advertise<r2sim::commander_plot>("commander_plot",5);
	ros::Publisher control_sp_pub = n.advertise<r2sim::control_sp>("control_sp",5);
	...
	while (ros::ok()){
		...
		if(cmd.last_flight_mode != cmd.flight_mode){
			...
			if(cmd.flight_mode == RASP_NURBS_SEMI||cmd.flight_mode == RASP_NURBS_AUTO){
				...
				trpz1.trpz_gen(nurbs1._len, 2.5, 15, 2.5);
				t_plan = 0.0;
				nbs.plan_ok = true;
				printf("traj generated\n");
			}
		}
		...
		//	printf("vel plan start\n");
			...
			printf("acc: %f\n", javp(1));
			printf("\n");
			r2sim::commander_plot commander_plot_msg;
			commander_plot_msg.inv_time = t*100;
			commander_plot_msg.j = javp(0)*100;
			commander_plot_msg.a = javp(1)*100;
			commander_plot_msg.v = javp(2)*100;
			commander_plot_msg.p = javp(3)*100;
			commander_plot_pub.publish(commander_plot_msg);
		}
		...
		//publish the sp
		r2sim::control_sp control_sp_msg;
		for(int i = 0; i < 3; i++){
